        ...
        <div class="bg-white dark:bg-dark-50 rounded-xl shadow-sm border border-light-200 dark:border-dark-200/50 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-dark dark:text-white mt-1">{{ $settings->currency }}{{ number_format($deposited, 2, '.', ',') }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-dark-50 rounded-xl shadow-sm border border-light-200 dark:border-dark-200/50 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-dark dark:text-white mt-1">{{ $settings->currency }}0.00</p>
                </div>
        ...
        <div class="bg-white dark:bg-dark-50 rounded-xl shadow-sm border border-light-200 dark:border-dark-200/50 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-dark dark:text-white mt-1">{{ isset($last_deposit) ? $settings->currency . number_format($last_deposit->amount, 2, '.', ',') : $settings->currency.'0.00' }}</p>
                </div>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-light-200 dark:divide-dark-200/50">
                ...
                <tr class="hover:bg-light-50 dark:hover:bg-dark-100/50 transition-colors text-sm">
                        ...
                        <div class="flex items-center">
                            @if (!empty($method->img_url))
                                <div class="w-8 h-8 rounded-lg bg-light-100 dark:bg-dark-200 p-1.5 mr-3 flex items-center justify-center">
                                    <img src="{{ $method->img_url }}" alt="{{ $method->name }}" class="h-full w-full object-contain">
                                </div>
                            @else
                                <div class="w-8 h-8 rounded-lg bg-primary-50 dark:bg-primary-900/30 p-1.5 mr-3 flex items-center justify-center">
                                    <svg class="w-5 h-5 text-primary-600 dark:text-primary-400" viewBox="0 0 24 24" fill="none">
                                        <path d="M9.5 13.7502C9.5 14.7202 10.25 15.5002 11.17 15.5002H13.05C13.85 15.5002 14.5 14.8202 14.5 13.9702C14.5 13.0602 14.1 12.7302 13.51 12.5202L10.5 11.4702C9.91 11.2602 9.51 10.9402 9.51 10.0202C9.51 9.18023 10.16 8.49023 10.96 8.49023H12.84C13.76 8.49023 14.51 9.27023 14.51 10.2402" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M12 7.5V16.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M22 12C22 17.52 17.52 22 12 22C6.48 22 2 17.52 2 12C2 6.48 6.48 2 12 2" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            ...
                            @endif
                            <div>
                                <p class="font-medium text-dark dark:text-white">{{ $method->name }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="text-dark dark:text-white">
                            <div class="text-xs text-dark-300 dark:text-light-300">Min: <span class="font-medium text-dark dark:text-white">{{ $settings->currency }}{{ number_format($moresettings->minamt, 2) }}</span></div>
                            @if(isset($method->max_amount))
                            <div class="text-xs text-dark-300 dark:text-light-300">Max: <span class="font-medium text-dark dark:text-white">{{ $settings->currency }}{{ number_format($method->max_amount, 2) }}</span></div>
                            @endif
                        </div>
                    </td>
                    <!--<td class="px-6 py-4 text-right">-->
                    ...
                    <!--        @if(isset($method->charges_amount))-->
                    ...
                    <!--</td>-->
                    <td class="px-6 py-4 text-right">
                        <button onclick="checkpamethd('{{ $method->id }}')" class="px-3 py-1.5 text-xs font-medium rounded-md bg-primary text-white hover:bg-primary-600 transition-colors">
                            Deposit
                        </button>
                    </td>
                </tr>
                    ...
                    <td colspan="5" class="px-6 py-8 text-center">
                        <div class="flex flex-col items-center justify-center">
                            <div class="w-16 h-16 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center mb-4">
                                <svg class="w-8 h-8 text-amber-600 dark:text-amber-400" viewBox="0 0 24 24" fill="none">
                            ...
                            <h4 class="text-xl font-semibold text-dark dark:text-white mb-2">No Payment Methods Available</h4>
                            <p class="text-sm text-dark-300 dark:text-light-300 mb-4 text-center max-w-md">
            ...
            <div class="space-y-2">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center">
                        ...
                        <span class="text-sm font-medium text-dark dark:text-white">{{ $settings->currency }}{{ number_format($moresettings->minamt, 2) }}</span>
                    </div>
                </div>
        ...
        <div class="p-5">
            <div class="space-y-4">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div>
                        <span class="text-xs text-dark dark:text-white font-medium">{{ $settings->currency }}{{ number_format($moresettings->minamt, 2) }}</span>
                    </div>
                </div>
            ...
            <div class="mt-5">
                <a href="{{ route('accounthistory') }}" class="text-xs text-primary dark:text-primary-400 font-medium flex items-center hover:underline">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#submitpaymentform').attr('action', "{{url('/dashboard/newdeposit')}}");
    });
</script>
