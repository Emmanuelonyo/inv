                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-5">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium dark:text-white text-dark">{{ \App\Models\BonusPercentage::getPercentByPlanId($planSelected->id) }}%</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="rounded-xl overflow-hidden border border-light-200/50 dark:border-dark-200/50 bg-light-100/50 dark:bg-dark-100/50 p-4">
                        ...
                        <div class="space-y-2">
                            @php
                                // -------------------------------
                                // 1) Parse & normalize duration → days
                                // -------------------------------
                                $durationNum  = (int) trim(preg_replace('/[^0-9]/', '', $planSelected->expiration));
                                $expLower     = strtolower($planSelected->expiration);
                            
                                if (str_contains($expLower, 'year')) {
                                    $durationDays = $durationNum * 365;
                                    $durationUnit = 'years';
                                } elseif (str_contains($expLower, 'month')) {
                                    $durationDays = $durationNum * 30;     // business rule: 30-day months
                                    $durationUnit = 'months';
                                } elseif (str_contains($expLower, 'week')) {
                                    $durationDays = $durationNum * 7;
                                    $durationUnit = 'weeks';
                                } else {
                                ...
                                // 2) Compute returns
                                // -------------------------------
                                $principal   = (float) $amountToInvest;
                                $finalBalance = $principal;      // will be set below
                                $returnOnly   = 0.0;             // profit excluding principal
                            
                                // Helper: convert an interval percent to DAILY compound rate
                                $percent      = (float) $planSelected->increment_amount;      // e.g. 1.95 (not 0.0195)
                                $interval     = strtolower($planSelected->increment_interval); // daily/weekly/monthly/yearly
                                $type         = strtolower($planSelected->increment_type);     // 'fixed' or 'percentage'
                            
                                // Normalize interval strings
                                if (in_array($interval, ['day', 'daily']))   $interval = 'daily';
                                if (in_array($interval, ['week', 'weekly'])) $interval = 'weekly';
                                if (in_array($interval, ['month', 'monthly'])) $interval = 'monthly';
                                if (in_array($interval, ['year', 'yearly', 'annually'])) $interval = 'yearly';
                            
                                if ($type === 'percentage') {
                                    // ---- Percentage plan with compounding ----
                                    // Convert the plan's interval % to an equivalent DAILY rate
                                    // r_daily = (1 + r_interval)^(1/n) - 1
                                    // where n = 1 (daily), 7 (weekly), 30 (monthly), 365 (yearly)
                                    $r_interval = $percent / 100.0;
                                    ...
                                    $r_daily = pow(1 + $r_interval, 1 / $n) - 1;
                            
                                    // Compound for durationDays
                                    $finalBalance = $principal * pow(1 + $r_daily, $durationDays);
                                    $returnOnly   = $finalBalance - $principal;
                            
                                } else {
                                    // ---- Fixed-amount plan (no compounding) ----
                                    // Add a fixed currency amount per interval
                                    $fixedAmount = (float) $planSelected->increment_amount; // e.g. $10 per interval
                                    ...
                                    $intervals = intdiv($durationDays, $intervalDays); // whole completed intervals
                                    $returnOnly = $fixedAmount * $intervals;
                                    $finalBalance = $principal + $returnOnly;
                                }
                            
                                // -------------------------------
                                // 3) Bonus / Gift
                                // -------------------------------
                                $bonusPercent = (float) (\App\Models\BonusPercentage::getPercentByPlanId($planSelected->id) ?? 0);
                                //echo $amountToInvest;
                                //echo $bonusPercent;
                                $bonus        = ($amountToInvest * $bonusPercent) / 100;
                            
                                // If the model tracks an existing gift, include it
                                $existingGift = (float) ($planSelected->gift ?? 0);
                                $planSelected->gift = $bonus;
                            
                                // -------------------------------
                                // 4) Totals for display
                                // -------------------------------
                                $totalReturn = $returnOnly;                               // profit from plan only
                                $totalProfit = $returnOnly + $planSelected->gift;         // plan profit + bonus
                                $finalAmount = $principal + $totalProfit;                 // principal + totalProfit
                            
                                // For label (show the raw configured % for clarity)
                                $avgReturnPercentage = $percent;                                    // as configured
                            @endphp
                            
                            <div class="flex justify-between items-center text-sm">
                                <span class="dark:text-gray-400 text-gray-600">Trading Capital:</span>
                            ...
                            <div class="flex justify-between items-center text-sm">
                                ...
                                <span class="font-medium text-green-500">+{{ $settings->currency }}{{ number_format($totalReturn, 3) }}</span>
                            </div>
                            ...
                            <div class="mt-3 pt-3 border-t border-light-200/50 dark:border-dark-200/50">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="font-bold text-lg text-primary">{{ $settings->currency }}{{ number_format($finalAmount, 3) }}</span>
                                </div>
                ...
                <div>
                    ...
                    <div class="rounded-xl overflow-hidden border border-light-200/50 dark:border-dark-200/50 bg-light-100/50 dark:bg-dark-100/50">
                        <div class="p-4 border-b border-light-200/50 dark:border-dark-200/50">
                            <p class="font-medium dark:text-white text-dark">Trading Summary</p>
                        </div>
                        <div class="p-4">
                            <div class="space-y-3">
                                <div class="flex justify-between items-center">
                                ...
                                <div class="flex justify-between items-center">
                                    <span class="text-sm dark:text-gray-400 text-gray-600">Trading Capital:</span>
    ...
    @else
        ...
        <div class="rounded-xl overflow-hidden bg-gradient-to-br from-white/50 to-white/20 dark:from-dark-50/50 dark:to-dark-50/20 border border-light-200/50 dark:border-dark-200/50">
            <div class="p-8 text-center">
                ...
                <h3 class="text-xl font-semibold dark:text-white text-dark mb-2">No Trading Plans Available</h3>
                <p class="text-gray-600 dark:text-gray-400 max-w-md mx-auto mb-6">
                    We're currently updating our trading offerings to bring you better opportunities. Please check back soon or contact our support team for assistance.
                </p>
