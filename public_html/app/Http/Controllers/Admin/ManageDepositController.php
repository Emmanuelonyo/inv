...
class ManageDepositController extends Controller
{
    ...
    public function deldeposit($id)
    {
        ...
        // Create notification for deleted deposit
        $formattedAmount = $settings->currency . number_format($deposit->amount, 2);
        $this->notificationService->createNotification(
            $user->id,
            "Your deposit request of {$formattedAmount} has been deleted by the administrator.",
            "Deposit Request Deleted",
            'warning',
            route('deposits'),
            'fa-trash',
            'bg-warning/10'
    ...
    if($deposit->user==$user->id){
            ...
            // Add notification for deposit bonus if applicable
            if ($settings->deposit_bonus != NULL and $settings->deposit_bonus > 0) {
                $bonus = $deposit->amount * $settings->deposit_bonus / 100;
                $formattedBonus = $settings->currency . number_format($bonus, 2);
                
                $this->notificationService->createNotification(
                    $user->id,
                    "You received a deposit bonus of {$formattedBonus} for your deposit of {$formattedAmount}",
                    "Deposit Bonus",
                    'success',
            ...
            if(!empty($user->ref_by)){
                ...
                $formattedEarnings = $settings->currency . number_format($earnings, 2);
                $this->notificationService->createNotification(
                    $agent->id,
                    "You received a referral commission of {$formattedEarnings} from {$user->name}'s deposit.",
                    "Referral Commission",
                    'success',
                    route('referuser'),
                    'fa-users',
